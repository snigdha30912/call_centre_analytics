...
c = 0
def speaker_separation(audio_file):
    ...
    separated_speaker_id = [i for i, x in enumerate(labels) if x == 1]
    
    print(separated_speaker_id)
    infiles = []
    for id in separated_speaker_id:
        path = 'chunk-%002d.wav'%(id,)
        infiles.append(path)
    ...
    for i in range(0,c):
        rm_path = 'chunk-%002d.wav'%(i,)
        os.remove(rm_path)
    return outfile


# speaker_separation('uploads/input_test.wav')